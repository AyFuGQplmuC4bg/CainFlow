...
void main() {
  ...
  test('card height grows with extra port rows', () {
    ...
    expect(
      nodeCardHeight(fourPort),
      greaterThan(nodeCardHeight(twoPort)),
    );
  });

  test('later input ports anchor lower than earlier ones', () {
    final gen = nodeRegistry.get('ImageGenerate');
    final first = portAnchorY(gen, isOutput: false, portIndex: 0, hasImage: false);
    final last = portAnchorY(gen, isOutput: false, portIndex: 2, hasImage: false);
    expect(last, greaterThan(first));
  });

  test('thumbnail increases card height and shifts anchors down', () {
    final gen = nodeRegistry.get('ImageGenerate');
    final noImg = portAnchorY(gen, isOutput: true, portIndex: 0, hasImage: false);
    final withImg = portAnchorY(gen, isOutput: true, portIndex: 0, hasImage: true);
